...
#[utoipa::path(
    ...
    description = "Previews the effective protocol mappers, roles and token claims (access token, ID token, userinfo) a user would receive from this client for a given scope set — without issuing (signing or persisting) a real token. When `user_id` is omitted, user-attribute mappers resolve to placeholder values.",
    ...
    responses(
        (status = 200, body = EvaluateClientScopesResult, description = "Token preview"),
