...
mod tests {
    ...
    async fn init_shared_ctx() -> SharedContext {
        ...
        let schema = format!("role_cross_realm_test_{}", Uuid::new_v4().simple());
    ...
    fn getting_a_role_from_another_realm_is_not_found() {
        rt().block_on(async {
            ...
            let response = server
                .get(&format!("/realms/{}/roles/{}", realm(), foreign_role_id))
            ...
            assert_eq!(
                ...
                404,
                "a role from another realm must not be readable through the principal realm: {}",
                response.text()
            );
        });
    }
    ...
    #[test]
    #[ignore = "requires PostgreSQL — run with: cargo test -p ferriskey-api --test role_cross_realm_test -- --ignored"]
    fn deleting_a_role_from_another_realm_is_not_found() {
        rt().block_on(async {
            let server = make_server();
            let token = get_admin_token(&server).await;
            let foreign_role_id = shared_ctx().neighbour_role_id;
            ...
            let response = server
                .delete(&format!("/realms/{}/roles/{}", realm(), foreign_role_id))
                .add_header("Authorization", auth_header(&token))
                .await;
