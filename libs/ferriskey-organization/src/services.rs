    ...
    OMR: OrganizationMemberRepository,
{
    async fn create_organization(
        ...
        let org = crate::Organization::new(org_config).map_err(|_| CoreError::Invalid)?;

        self.organization_repository
            .create_organization(crate::CreateOrganizationParams {
        ...
        user_repo.expect_get_by_id().returning(move |id| {
            ...
            Box::pin(async move {
                if id == admin_clone.id {
                    Ok(Unscoped::new(admin_clone))
                } else {
                    Ok(Unscoped::new(member_clone))
                }
            })
        });
