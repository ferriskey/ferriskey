...
mod test {
    ...
    #[tokio::test]
    async fn auth_accepts_regex_redirect_uri() {
        // Arrange
        let mut realm_repo = get_mock_realm_repository_with_clone_expectations();
        let mut client_repo = get_mock_client_repository_with_clone_expectations();
        let mut redirect_repo = get_mock_redirect_uri_repository_with_clone_expectations();
        ...
        // regex redirect allowing extra query params
        let redirect_pattern = r"^https://example\\.com/callback(\?.*)?$".to_string();
        let redirect = RedirectUri::new(client.id, redirect_pattern.clone(), true);
        ...
        client_repo
            .expect_get_by_client_id()
            .returning(move |_cid: String, _rid: Uuid| Box::pin(ready(Ok(client_clone.clone()))));
        let client_id_for_redirect = client.id;
        redirect_repo
            .expect_get_enabled_by_client_id()
            .returning(move |id: Uuid| {
                assert_eq!(id, client_id_for_redirect);
                Box::pin(ready(Ok(vec![redirect.clone()])))
            });
        session_repo
            .expect_create()
        ...
        // Assert
        assert!(out.login_url.contains("client_id=client123"));
        assert!(out.login_url.contains("redirect_uri=https://example.com/callback?foo=bar"));
        assert!(out.login_url.contains("state=state123"));
    }
