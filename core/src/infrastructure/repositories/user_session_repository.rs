...
impl From<crate::infrastructure::entities::user_sessions::Model> for UserSession {
    fn from(model: crate::infrastructure::entities::user_sessions::Model) -> Self {
...
impl UserSessionRepository for PostgresUserSessionRepository {
    async fn create(&self, session: &UserSession) -> Result<(), SessionError> {
        let active_model = crate::infrastructure::entities::user_sessions::ActiveModel {
            id: Set(session.id),
    ...
    async fn find_by_user_id(&self, user_id: &Uuid) -> Result<UserSession, SessionError> {
        let user_session = crate::infrastructure::entities::user_sessions::Entity::find()
            .filter(crate::infrastructure::entities::user_sessions::Column::UserId.eq(*user_id))
            .one(&self.db)
            .await
    ...
    async fn delete(&self, id: &Uuid) -> Result<(), SessionError> {
        crate::infrastructure::entities::user_sessions::Entity::delete_by_id(*id)
            .exec(&self.db)
            .await
