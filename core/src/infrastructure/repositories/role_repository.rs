...
use crate::infrastructure::entities::roles::{Entity as RoleEntity, Model as RoleModel, ActiveModel};
...
impl RoleRepository for PostgresRoleRepository {
    ...
    async fn get_by_client_id(&self, client_id: uuid::Uuid) -> Result<Vec<Role>, RoleError> {
        let roles = RoleEntity::find()
            .filter(crate::infrastructure::entities::roles::Column::ClientId.eq(client_id))
            .all(&self.db)
            .await
            .map_err(|_| RoleError::InternalServerError)?
            .iter()
    ...
    async fn get_by_id(&self, id: Uuid) -> Result<Option<Role>, RoleError> {
        let roles_with_clients = RoleEntity::find()
            .filter(crate::infrastructure::entities::roles::Column::Id.eq(id))
            .find_with_related(crate::infrastructure::entities::clients::Entity)
            .all(&self.db)
            .await
            .map_err(|_| RoleError::InternalServerError)?;
    ...
    async fn delete_by_id(&self, id: uuid::Uuid) -> Result<(), RoleError> {
        let result = RoleEntity::delete_many()
            .filter(crate::infrastructure::entities::roles::Column::Id.eq(id))
            .exec(&self.db)
            .await
            .map_err(|_| RoleError::InternalServerError)?;
    ...
    async fn find_by_realm_id(&self, realm_id: Uuid) -> Result<Vec<Role>, RoleError> {
        let roles = RoleEntity::find()
            .filter(crate::infrastructure::entities::roles::Column::RealmId.eq(realm_id))
            .find_with_related(crate::infrastructure::entities::clients::Entity)
            .all(&self.db)
            .await
    ...
    async fn find_by_name(&self, name: String, realm_id: Uuid) -> Result<Option<Role>, RoleError> {
        let role = RoleEntity::find()
            .filter(crate::infrastructure::entities::roles::Column::Name.eq(name))
            .filter(crate::infrastructure::entities::roles::Column::RealmId.eq(realm_id))
            .one(&self.db)
            .await
            .map_err(|_| RoleError::InternalServerError)?
    ...
    async fn update_by_id(&self, id: Uuid, payload: UpdateRoleRequest) -> Result<Role, RoleError> {
        ...
        let mut role: ActiveModel = role.into();
