...
use uuid::Uuid;
use crate::infrastructure::entities::refresh_tokens::{Entity as RefreshTokenEntity, Model as RefreshTokenModel};
...
impl RefreshTokenRepository for PostgresRefreshTokenRepository {
    ...
    async fn get_by_jti(&self, jti: Uuid) -> Result<RefreshToken, JwtError> {
        let refresh_token = RefreshTokenEntity::find()
            .filter(crate::infrastructure::entities::refresh_tokens::Column::Jti.eq(jti))
            .one(&self.db)
            .await
            .map_err(|e| JwtError::GenerationError(e.to_string()))?
