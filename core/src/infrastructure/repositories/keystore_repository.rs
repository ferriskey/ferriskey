...
impl TryFrom<crate::infrastructure::entities::jwt_keys::Model> for JwtKeyPair {
    type Error = JwtError;

    fn try_from(value: crate::infrastructure::entities::jwt_keys::Model) -> Result<Self, Self::Error> {
...
impl KeyStoreRepository for PostgresKeyStoreRepository {
    async fn get_or_generate_key(&self, realm_id: Uuid) -> Result<JwtKeyPair, JwtError> {
        let key = crate::infrastructure::entities::jwt_keys::Entity::find()
            .filter(crate::infrastructure::entities::jwt_keys::Column::RealmId.eq(realm_id))
            .one(&self.db)
            .await
        ...
        let new_key = crate::infrastructure::entities::jwt_keys::ActiveModel {
            id: Set(id),
