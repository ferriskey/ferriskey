...
impl UserRepository for PostgresUserRepository {
    async fn create_user(&self, dto: CreateUserRequest) -> Result<User, UserError> {
        ...
        let model = crate::infrastructure::entities::users::ActiveModel {
            id: Set(user.id),
    ...
    async fn get_by_username(&self, username: String, realm_id: Uuid) -> Result<User, UserError> {
        let users_model = crate::infrastructure::entities::users::Entity::find()
            .filter(crate::infrastructure::entities::users::Column::Username.eq(username.clone()))
            .filter(crate::infrastructure::entities::users::Column::RealmId.eq(realm_id))
            .find_also_related(crate::infrastructure::entities::realms::Entity)
            .all(&self.db)
            .await
            .map_err(|e| {
        ...
        let required_actions: Vec<RequiredAction> = user_model
            .find_related(crate::infrastructure::entities::user_required_actions::Entity)
            .all(&self.db)
            .await
            .map_err(|_| UserError::InternalServerError)?
            .into_iter()
    ...
    async fn get_by_client_id(&self, client_id: Uuid) -> Result<User, UserError> {
        let user = crate::infrastructure::entities::users::Entity::find()
            .filter(crate::infrastructure::entities::users::Column::ClientId.eq(client_id))
            .one(&self.db)
            .await
    ...
    async fn get_by_id(&self, id: Uuid) -> Result<User, UserError> {
        let users_model = crate::infrastructure::entities::users::Entity::find()
            .filter(crate::infrastructure::entities::users::Column::Id.eq(id))
            .find_also_related(crate::infrastructure::entities::realms::Entity)
            .all(&self.db)
            .await
            .map_err(|e| {
    ...
    async fn find_by_realm_id(&self, realm_id: Uuid) -> Result<Vec<User>, UserError> {
        let users = crate::infrastructure::entities::users::Entity::find()
            .filter(crate::infrastructure::entities::users::Column::RealmId.eq(realm_id))
            .all(&self.db)
            .await
            .map_err(|_| UserError::NotFound)?;
    ...
    async fn bulk_delete_user(&self, ids: Vec<Uuid>) -> Result<u64, UserError> {
        let rows = crate::infrastructure::entities::users::Entity::delete_many()
            .filter(
                Condition::all()
                    .add(crate::infrastructure::entities::users::Column::Id.is_in(ids.clone()))
                    .add(crate::infrastructure::entities::users::Column::ClientId.is_null()),
            )
            .exec(&self.db)
            .await
            .map_err(|e| {
    ...
    async fn delete_user(&self, user_id: Uuid) -> Result<u64, UserError> {
        let rows = crate::infrastructure::entities::users::Entity::delete_by_id(user_id)
            .exec(&self.db)
            .await
            .map_err(|_| UserError::InternalServerError)?;
    ...
    async fn update_user(&self, user_id: Uuid, dto: UpdateUserRequest) -> Result<User, UserError> {
        let user = crate::infrastructure::entities::users::Entity::find()
            .filter(crate::infrastructure::entities::users::Column::Id.eq(user_id))
            .one(&self.db)
            .await
            .map_err(|_| UserError::NotFound)?;
        ...
        let mut active_model: crate::infrastructure::entities::users::ActiveModel = user.into();
