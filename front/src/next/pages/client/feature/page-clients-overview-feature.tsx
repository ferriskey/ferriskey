...
import { NEXT_CLIENT_CREATE_URL, NEXT_CLIENT_URL } from '../client-routes'
  ...
  return (
    <PageClientsOverview
      ...
      createUrl={(protocol: ClientProtocol) =>
        `${NEXT_CLIENT_CREATE_URL(realm)}?protocol=${protocol}`
      }
      clientHref={(client: Client) => `${NEXT_CLIENT_URL(realm, client.id)}/settings`}
    />
  )
}
