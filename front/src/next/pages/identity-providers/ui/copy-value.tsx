...
export default function CopyValue({ value, label, className }: CopyValueProps) {
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-md border border-fk-line bg-neutral-50 px-2.5 py-1.5',
        className
      )}
    >
      <code className='min-w-0 flex-1 truncate font-mono-ui text-xs text-neutral-700'>
        {value}
      </code>
      <button
        type='button'
