...
function SubItems({
  ...
  section: NonNullable<ReturnType<typeof findActiveSection>>
  realmName: string
  pathname: string
}) {
  return (
    <ul className='flex flex-col gap-0.5'>
      {section.subItems.map((item) => {
        const active = isSubItemActive(pathname, realmName, item)
        return (
          <li key={item.key}>
            <NavLink
              to={subItemUrl(realmName, item)}
              className={cn(
  ...
  return (
    <TooltipProvider delayDuration={200}>
      <div className='flex h-screen flex-col overflow-hidden bg-white text-fk-ink dark:bg-fk-surface'>
        <TopBar
          realm={realm_name}
          homeHref={`${CONSOLE_URL(realm_name)}/activity/live`}
          realmHrefFor={(name) => `${CONSOLE_URL(name)}/activity/live`}
        ...
        <nav className='flex h-9 shrink-0 items-stretch gap-1 overflow-x-auto scrollbar-none border-b border-fk-line px-3'>
        ...
        <div className='flex min-h-0 flex-1 items-stretch'>
          {tier === 'desktop' && section && section.subItems.length > 0 && (
            <aside className='w-52 shrink-0 overflow-y-auto border-r border-fk-line p-2'>
